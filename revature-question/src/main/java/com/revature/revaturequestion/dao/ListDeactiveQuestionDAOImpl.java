...
@Repository
public class ListDeactiveQuestionDAOImpl implements IListDeactiveQuestionDAO {
	@Autowired
	...
	public List<Question> listDeactiveQuestions() throws SQLException {
		...
		try {
			...
			String sql = "select q.title,q.tag,c.name,ql.name from question q,category c,question_level ql where q.category_id=c.id and q.level_id=ql.id and q.status=0";
